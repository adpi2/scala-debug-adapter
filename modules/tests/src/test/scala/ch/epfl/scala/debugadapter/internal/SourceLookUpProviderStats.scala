...
class SourceLookUpProviderStats extends FunSuite {
  ...
  override def munitTimeout: Duration = 400.seconds
  // test("scaladex") {
  ...
  private def printAndCheck(project: String, entry: ClassEntry): Unit = {
    val (duration, lookUp) =
      TimeUtils.timed(SourceLookUpProvider(Seq(entry), PrintLogger))
